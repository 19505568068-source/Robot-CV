...
# Scheduled tasks do not inherit the interactive Codex PATH. Discover the
# installed CLI so the bridge can invoke Codex after login or reboot.
$codexBinRoot = Join-Path $env:LOCALAPPDATA "OpenAI\Codex\bin"
$codexBinDirectory = Get-ChildItem -LiteralPath $codexBinRoot -Directory -ErrorAction SilentlyContinue |
    Sort-Object LastWriteTime -Descending |
    Where-Object { Test-Path -LiteralPath (Join-Path $_.FullName "codex.exe") } |
    Select-Object -First 1
if ($codexBinDirectory) {
    $env:PATH = "$($codexBinDirectory.FullName);$env:PATH"
}

if ([string]::IsNullOrWhiteSpace($NodePath)) {
    $nvmNodePath = "C:\nvm4w\nodejs\node.exe"
    $codexRuntimeNodePath = Join-Path $env:USERPROFILE ".cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe"
    if (Test-Path -LiteralPath $nvmNodePath) {
        $NodePath = $nvmNodePath
    } elseif (Test-Path -LiteralPath $codexRuntimeNodePath) {
        $NodePath = $codexRuntimeNodePath
    } else {
...
if ($Watch) {
    Write-LauncherLog "Watchdog is active; checking every $WatchdogIntervalSeconds seconds."
    $restartParameters = @{
        NoOpen = $true
        NodePath = $NodePath
        EntryPath = $EntryPath
    ...
    while ($true) {
        ...
        Write-LauncherLog "Watchdog detected an unavailable service; restarting."
        try {
            & $PSCommandPath @restartParameters
        } catch {
            Write-LauncherLog "Watchdog restart failed: $($_.Exception.Message)"
        }
    }
}
