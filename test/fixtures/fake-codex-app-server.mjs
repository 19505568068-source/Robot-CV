...
const delayedInterruptTurns = new Set();
const secureWorkspace = process.env.CODEX_TEST_SECURE_WORKSPACE ?? "/tmp/empty-workspace";
...
rl.on("line", (line) => {
  ...
  if (!message.method && approvalResponses.has(message.id)) {
    ...
    const actualResponse = message.error === undefined ? { result: message.result } : { error: message.error };
    const expectedResponse = expected.error === undefined ? { result: expected.result } : { error: expected.error };
    if (JSON.stringify(actualResponse) !== JSON.stringify(expectedResponse)) {
      process.stderr.write(`unexpected approval response for ${message.id}: ${JSON.stringify(actualResponse)}\n`);
      process.exit(2);
      return;
    }
    if (!approvalResponses.size) {
      completePrompt(expected.threadId, expected.turnId, expected.prompt);
    }
    return;
  }
  ...
  if (message.method === "thread/start") {
    ...
    if (process.env.CODEX_TEST_REQUIRE_SECURE_OPTIONS === "1") {
      if (message.params?.permissions !== "hr-public-read-only"
        || message.params?.sandbox !== undefined
        || JSON.stringify(message.params?.runtimeWorkspaceRoots) !== JSON.stringify([secureWorkspace])
        || JSON.stringify(message.params?.environments) !== "[]"
        || JSON.stringify(message.params?.dynamicTools) !== "[]"
        || JSON.stringify(message.params?.selectedCapabilityRoots) !== "[]") {
        fail(message.id, "thread/start requires the restricted permission profile");
        return;
      }
    }
    respond(message.id, {
  ...
  if (message.method === "thread/resume") {
    if (process.env.CODEX_TEST_REQUIRE_SECURE_OPTIONS === "1") {
      if (message.params?.permissions !== "hr-public-read-only"
        || message.params?.sandbox !== undefined
        || JSON.stringify(message.params?.runtimeWorkspaceRoots) !== JSON.stringify([secureWorkspace])
        || message.params?.environments !== undefined
        || message.params?.dynamicTools !== undefined
        || message.params?.selectedCapabilityRoots !== undefined) {
        fail(message.id, "thread/resume requires the restricted permission profile");
        return;
      }
    }
    respond(message.id, {
  ...
  if (message.method === "turn/start") {
    ...
    if (process.env.CODEX_TEST_REQUIRE_SECURE_OPTIONS === "1") {
      ...
      if (message.params?.permissions !== "hr-public-read-only"
        ...
        || JSON.stringify(message.params?.runtimeWorkspaceRoots) !== JSON.stringify([secureWorkspace])
        || JSON.stringify(message.params?.environments) !== "[]") {
        fail(message.id, "turn/start requires the restricted permission profile");
        return;
      }
      if (JSON.stringify(message.params?.outputSchema) !== JSON.stringify(expectedOutputSchema)) {
        fail(message.id, "turn/start requires outputSchema");
        return;
      }
    }
    activeTurns.set(message.params.threadId, turnId);
    respond(message.id, { turn: completedTurn(turnId, "inProgress") });
    if (prompt === "hold" || prompt === "hold-delayed-interrupt") {
      if (prompt === "hold-delayed-interrupt") delayedInterruptTurns.add(turnId);
    ...
    if (prompt === "approval") {
      requestApprovals(message.params.threadId, turnId, false, prompt);
      return;
    }
    if (prompt === "denial") {
      requestApprovals(message.params.threadId, turnId, true, prompt);
      return;
    }
    setTimeout(() => {
  ...
  if (message.method === "turn/interrupt") {
    ...
    if (delayedInterruptTurns.delete(activeTurnId)) {
      respond(message.id, {});
      setTimeout(() => {
        send({
        ...
        if (activeTurns.get(message.params.threadId) === activeTurnId) {
...
function requestApprovals(threadId, turnId, denyAll, prompt) {
  const requests = [
    {
      id: "approve-command",
      method: "item/commandExecution/requestApproval",
      params: { threadId, turnId, itemId: "command-item", command: "test" },
      result: { decision: denyAll ? "decline" : "acceptForSession" }
    },
    {
      id: "approve-file",
    ...
    },
    {
      ...
      ...(denyAll
        ? { error: { code: -32000, message: "Permission escalation denied by client policy" } }
        : {
            result: {
              permissions: { network: { domains: { "127.0.0.1": "allow" } } },
              scope: "session"
  ...
  if (denyAll) {
    requests.push(
      {
        id: "deny-legacy-command",
        method: "execCommandApproval",
        params: { threadId, turnId, command: "test" },
        result: { decision: "denied" }
      },
      {
        id: "deny-legacy-file",
        method: "applyPatchApproval",
        params: { threadId, turnId, patch: "test" },
        result: { decision: "denied" }
      },
      {
        id: "deny-mcp",
        method: "mcpServer/elicitation/request",
        params: { threadId, turnId, serverName: "test" },
        result: { action: "cancel", content: null, _meta: null }
      },
      {
        id: "deny-dynamic-tool",
        method: "item/tool/call",
        params: { threadId, turnId, tool: "test", arguments: {} },
        result: {
  ...
  for (const request of requests) {
    approvalResponses.set(request.id, {
