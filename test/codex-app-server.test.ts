...
test("denies approvals, permissions, MCP elicitation, and dynamic tools in deny-all mode", async (t) => {
  const runner = new AppServerCodexRunner({
    codexBin: path.join(fixturesDir, "fake-codex-app-server.mjs"),
    requestTimeoutMs: 2_000,
    serverRequestPolicy: "deny-all"
  });
  t.after(() => runner.close());

  const result = await runner.run({ prompt: "denial", cwd: "/tmp/project" });

  assert.equal(result.text, "reply:denial");
});

test("sends a restricted read-only sandbox and output schema to app-server", async (t) => {
  const secureWorkspace = path.resolve(fixturesDir, "empty-workspace");
  const runner = new AppServerCodexRunner({
    codexBin: path.join(fixturesDir, "fake-codex-app-server.mjs"),
    requestTimeoutMs: 2_000,
    env: {
      ...process.env,
      CODEX_TEST_REQUIRE_SECURE_OPTIONS: "1",
      CODEX_TEST_SECURE_WORKSPACE: secureWorkspace
    }
  });
  t.after(() => runner.close());

  const result = await runner.run({
    prompt: "secure-options",
    cwd: secureWorkspace,
    sandboxPolicy: {
      type: "readOnly",
  ...
  assert.equal(result.text, "reply:secure-options");
});

test("bounds a whole app-server turn and interrupts its exact turn id", { timeout: 1_000 }, async (t) => {
  const runner = new AppServerCodexRunner({
    codexBin: path.join(fixturesDir, "fake-codex-app-server.mjs"),
    requestTimeoutMs: 2_000,
    turnTimeoutMs: 30
  ...
  await assert.rejects(
    runner.run({ prompt: "hold", cwd: "/tmp/project", threadId: "thread-turn-timeout" }),
    /turn timed out after 30ms/i
  );
});

test("a late cancelled completion cannot clear the next active turn", { timeout: 1_000 }, async (t) => {
  const runner = new AppServerCodexRunner({
    codexBin: path.join(fixturesDir, "fake-codex-app-server.mjs"),
    requestTimeoutMs: 2_000
  });
  ...
  await assert.rejects(
    runner.run({
      prompt: "hold-delayed-interrupt",
      cwd: "/tmp/project",
      threadId: "thread-delayed-completion",
      turnTimeoutMs: 20
    }),
    /turn timed out after 20ms/i
  );

  const nextTurn = runner.run({
    prompt: "hold",
    cwd: "/tmp/project",
    threadId: "thread-delayed-completion",
    turnTimeoutMs: 500
  ...
  await runner.stop("thread-delayed-completion");
  await assert.rejects(nextTurn, /interrupted/i);
});
...
test("auto backend cannot downgrade a restricted app-server contract to exec", async (t) => {
  const runner = new HybridCodexRunner({
    backend: "auto",
    codexBin: path.join(fixturesDir, "fake-codex-fallback.mjs"),
    timeoutMs: 2_000
  ...
  await assert.rejects(runner.run({
    ...
    sandboxPolicy: {
      ...
      access: {
        type: "restricted",
        includePlatformDefaults: false,
        readableRoots: [fixturesDir]
      }
    }
  }), /app-server exited|unavailable/i);
});
