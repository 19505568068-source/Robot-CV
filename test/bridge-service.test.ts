...
test("replaces a Codex thread that already has an active writer", async (t) => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "codex-weixin-writer-conflict-"));
  t.after(() => fs.rmSync(tmpDir, { recursive: true, force: true }));
  const stateStore = new RuntimeStateStore(resolveStatePaths(path.join(tmpDir, "state")));
  stateStore.ensureActiveSession("alice@im.wechat", tmpDir);
  stateStore.setThread("alice@im.wechat", "thread-busy");
  const attempts: Array<{ prompt: string; threadId?: string }> = [];
  const replies: string[] = [];
  let calls = 0;
  const service = new BridgeService({
    ...
    runner: {
      async run(input: { prompt: string; threadId?: string; onThreadStarted?: (threadId: string) => void }) {
        ...
        calls += 1;
        if (calls === 1) {
          throw new Error("thread/resume failed: thread thread-busy already has an active writer");
        }
        input.onThreadStarted?.("thread-replacement");
        return { raw: "", text: "已恢复", threadId: "thread-replacement" };
      },
      async stop() {}
    } as never
  });
  ...
  assert.equal(attempts[0]?.threadId, "thread-busy");
  assert.equal(attempts[1]?.threadId, undefined);
  assert.equal(attempts[1]?.prompt, attempts[0]?.prompt);
  assert.equal(stateStore.getThread("alice@im.wechat"), "thread-replacement");
  assert.deepEqual(replies, ["已恢复"]);
});
