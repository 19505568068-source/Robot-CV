...
test("recognizes an occupied Codex thread writer", () => {
  assert.equal(isThreadWriterConflict(new Error(
    "thread/resume failed: thread abc already has an active writer"
  )), true);
  assert.equal(isThreadWriterConflict(new Error("HTTP 503 Service Unavailable")), false);
});
