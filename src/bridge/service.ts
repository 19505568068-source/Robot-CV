...
import { isRecoverableMessageHandlingError, isThreadWriterConflict } from "./errors.js";
import {
  buildPrompt,
  buildPromptPreview,
  chunkText,
...
export class BridgeService {
  ...
  private async runWithUnclassifiedRetries(input: {
    ...
    let recoveredWriterConflict = false;
    ...
    while (true) {
      ...
      try {
        ...
        if (!input.isCancelled() && threadId && !recoveredWriterConflict && isThreadWriterConflict(error)) {
          console.warn(
            `[codex-weixin] thread ${threadId} is already in use; `
            + `starting a replacement thread for ${input.senderId}`
          );
          recoveredWriterConflict = true;
          retryAttempt += 1;
          threadId = undefined;
          this.options.stateStore.setThread(input.senderId, "");
