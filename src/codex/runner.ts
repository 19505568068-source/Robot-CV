...
export type HybridCodexRunnerOptions = {
  ...
  appServerProcessCwd?: string;
  appServerEnv?: NodeJS.ProcessEnv;
  appServerRequestPolicy?: AppServerRunnerOptions["serverRequestPolicy"];
  appServerStrictConfig?: boolean;
  appServerExperimentalApi?: boolean;
};
...
export class HybridCodexRunner {
  ...
  constructor(private readonly options: HybridCodexRunnerOptions) {
    this.appServer = new AppServerCodexRunner({
      ...
      processCwd: options.appServerProcessCwd,
      env: options.appServerEnv,
      serverRequestPolicy: options.appServerRequestPolicy,
      strictConfig: options.appServerStrictConfig,
      experimentalApi: options.appServerExperimentalApi
    });
  ...
  async run(input: CodexRunnerInput): Promise<CodexRunResult> {
    ...
    const requiresAppServerContract = Boolean(
      input.sandboxPolicy
      || input.outputSchema
      || input.signal
    ...
    if (this.options.backend === "exec" && !requiresAppServerForStreaming) {
      if (requiresAppServerContract) {
        throw new Error("Codex exec backend cannot satisfy the requested app-server security contract");
      }
