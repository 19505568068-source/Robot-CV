...
export const HR_WEB_PROJECTS: readonly HrWebProjectDetail[] = [
  {
    ...
    category: "个人项目 · 项目资料",
    summary: "把简历、知识问答、招聘对话和机会证据串成可审核的移动端流程。",
    dateLabel: "2026.09",
    statusLabel: "本机链路已验证 · 公网后端待接入",
    role: "需求与流程设计、开源框架二次开发、前后端整合、测试与部署边界验证",
    problem: "需要让 HR 通过同一个二维码完成授权、查看简历和多轮沟通，同时避免把管理后台、原始资料或候选人承诺暴露给公开访客端。",
    work: "基于 codex-weixin 0.3.10 拆分本机管理端与独立 H5 访客端；实现同意后介绍与原始简历交付、公司询问、项目入口、多轮 Codex 回答、聊天归档和机会证据留存，并整理完整前后端源码与 Cloudflare 静态入口配置。",
    method: "先划分公开访客面与本机数据边界，再用独立会话、CSRF、来源校验和限流保护入口；同一访客通过队列保持消息顺序，公司、岗位、条件、关注点和邀约信号按结构化证据记录，重要承诺保留人工确认。",
    result: "本机与同一局域网的完整链路已验证，公开访客链路的定向自动化测试与类型检查通过；Cloudflare 当前只能承载 H5 静态外壳，/api/public/* 仍需可信 HTTPS 反向代理或 Tunnel 接入可达的 Node 后端，不能表述为已完成公网全栈部署。原始简历保持不被覆盖。"
  },
  {
    id: "red-infinity",
...
export function formatHrWebProjectDetail(project: HrWebProjectDetail): string {
  return [
    ...
    "以上是我在该项目中的角色、具体工作和结果，请从这段经历开始提问。"
  ].join("\n\n");
}
